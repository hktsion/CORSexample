var timeApp = (function(d) {
  	...
  	function mostrarPropsOtrasCiudades(j){		
		...
		for(let i in j){
			if(j[i].hasOwnProperty('UTC')){
				...
				d.querySelector('#ciudades').appendChild(span);

				for(let i= 0; i < (d.querySelector('#ciudades').childElementCount); i++ ){
					d.querySelector('#ciudades > span:nth-child('+(i+1)+')').textContent = j[i]['ciudad'];
				}
			}
		}
		return;
	...
	function getHoraLocal(utc){
		let ifecha = (new Date()).getTime() + (utc*60*60);
